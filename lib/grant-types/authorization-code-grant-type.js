...
AuthorizationCodeGrantType.prototype.saveToken = async function(user, client, authorizationCode, scope) {
  const validatedScope = !scope ? scope : await this.validateScope(user, client, scope);
  const fns = [
