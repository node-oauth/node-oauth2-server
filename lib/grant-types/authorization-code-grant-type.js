...
AuthorizationCodeGrantType.prototype.saveToken = function(user, client, authorizationCode, scope) {
  const self = this;
  function validateParamScope() {
    if (!scope) return Promise.resolve(scope);
    return self.validateScope(user, client, scope);
  }
  function doActualSave(validatedScope) {
    const fns = [
      self.generateAccessToken(client, user, validatedScope),
      self.generateRefreshToken(client, user, validatedScope),
      self.getAccessTokenExpiresAt(),
      self.getRefreshTokenExpiresAt()
    ];
    ...
    return Promise.all(fns)
      ...
      .spread(function(accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt) {
        ...
        return promisify(self.model.saveToken, 3).call(self.model, token, client, user);
      });
  }
  return validateParamScope().then(doActualSave);
};
