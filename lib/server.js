...
class OAuth2Server {
   ...
   * **Remarks:**
   *
   * Any valid option for `OAuth2Server#authenticate() <OAuth2Server#authenticate>`, `OAuth2Server#authorize() <OAuth2Server#authorize>` and `OAuth2Server#token() <OAuth2Server#token>` can be passed to the constructor as well.
   ...
   *
   * @param options.scope {string[]=} The scope(s) to authenticate.
   ...
   * @param [options.requireClientAuthentication={}] {object|boolean} Require a client secret (see remarks section). Defaults to `true` for all grant types.
   ...
   * @param [options.extendedGrantTypes={}] {object=} Additional supported grant types.
   *
   * @throws {InvalidArgumentError} if the model is missing
   * @return {object} A new `OAuth2Server` instance.
   * @example
   * // Basic usage:
   ...
   * @return {Promise.<object>} A `Promise` that resolves to the access token object returned from `Model#getAccessToken()`.
   *   In case of an error, the promise rejects with one of the error types derived from `/api/errors/oauth-error`.
   * @example
   ...
   * **Remarks:**
   *
   * If `request.query.allowed` equals the string `'false'` the access request is denied and the returned promise is rejected with an `/api/errors/access-denied-error`.
   *
   ...
   * @return {Promise.<object>} A `Promise` that resolves to the authorization code object returned from `Model#saveAuthorizationCode() <Model#saveAuthorizationCode>`.
   *   In case of an error, the promise rejects with one of the error types derived from `/api/errors/oauth-error`.
   * @example
   * const oauth = new OAuth2Server({model: ...});
   ...
   * @param [options.alwaysIssueNewRefreshToken=true] {boolean=} Always revoke the used refresh token and issue a new one for the `refresh_token` grant.
   * @param [options.extendedGrantTypes={}] {object=} Additional supported grant types.
   * @return {Promise.<object>} A `Promise` that resolves to the token object returned from `Model#saveToken() <Model#saveToken>`.
   *   In case of an error, the promise rejects with one of the error types derived from `/api/errors/oauth-error`.
