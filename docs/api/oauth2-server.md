...
| options.model | Object | The `Model </model/spec>`. |
...
Any valid option for `OAuth2Server#authenticate() <OAuth2Server#authenticate>`, `OAuth2Server#authorize() <OAuth2Server#authorize>` and `OAuth2Server#token() <OAuth2Server#token>` can be passed to the constructor as well. The supplied options will be used as default for the other methods.
...
A `Promise` that resolves to the access token object returned from `Model#getAccessToken() <Model#getAccessToken>`.
...
`/api/errors/unauthorized-request-error`:  
...
A `Promise` that resolves to the authorization code object returned from `Model#saveAuthorizationCode() <Model#saveAuthorizationCode>`.
In case of an error, the promise rejects with one of the error types derived from `/api/errors/oauth-error`.

Possible errors include but are not limited to:

`/api/errors/access-denied-error`  
...
A `Promise` that resolves to the token object returned from `Model#saveToken() <Model#saveToken>`.
In case of an error, the promise rejects with one of the error types derived from `/api/errors/oauth-error`.

Possible errors include but are not limited to:

`/api/errors/invalid-grant-error`:  
...
For information on how to implement a handler for a custom grant type see `/misc/extension-grants`.
