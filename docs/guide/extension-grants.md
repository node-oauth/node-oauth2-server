...
Create a subclass of `AbstractGrantType` and create methods <span class="title-ref">handle</span> and <span class="title-ref">saveToken</span> along with other required methods according to your needs:
...
Extension grants are registered through `OAuth2Server#token() <OAuth2Server#token>` (`options.extendedGrantTypes`).
